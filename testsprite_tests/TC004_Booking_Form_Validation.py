...
async def run_test():
    ...
    try:
        ...
        # -> Click the first 'Reservar Cupo' button on the first class card to open the booking form (use element index 167).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div/div/main/div/div/div[3]/div[1]/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Confirmar Reserva' submit button (index 381) with all fields empty to trigger validation; then fill only the name field (index 353) and click submit again to verify email is still required.
        frame = context.pages[-1]
        # Click element
